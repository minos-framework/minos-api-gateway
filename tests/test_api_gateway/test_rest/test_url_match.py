...
from minos.api_gateway.rest.urlmatch.urlmatch import (
    BadMatchPattern,
    UrlMatch,
)
...
class URLMatchTest(unittest.TestCase):
    ...
    def _check_raises(self, pattern):
        ...
        with self.assertRaises(BadMatchPattern):
            UrlMatch.urlmatch(pattern, self._http_url)
    ...
    def _check_urls(self, pattern, urls, expected_result, **kwargs):
        ...
        for url in urls:
            result = UrlMatch.urlmatch(pattern, url, **kwargs)
    ...
    def test_match_exact(self):
        ...
        self.assertEqual(UrlMatch.urlmatch(pattern, "http://test.com/exact/path"), True)

        bad_urls = ["http://test.com/inexact/path", "http://badtest.com/exact/path"]
